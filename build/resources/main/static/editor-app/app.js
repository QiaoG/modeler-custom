  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
            ...
            function fetchModel(modelId) {
                ...
                console.log('by modelId=' + modelId + ' get modelUrl=' + modelUrl + ' ');
                $http({method: 'GET', url: modelUrl}).
                    success(function (data, status, headers, config) {
                        console.log('loading model with id ' + modelId + ' ' + modelUrl + ' ' + data);
                        $rootScope.editor = new ORYX.Editor(data);
                        $rootScope.modelData = angular.fromJson(data);
                        $rootScope.editorFactory.resolve();
                    }).
                    error(function (data, status, headers, config) {
                      console.log('Error loading model by id ' + modelId + ' ' + modelUrl + data);
                      /*var d = {name:'demo',revision:'1',description:'',modelId:'1'
                          ,model:{id:'canvas',resourceId:'canvas',stencilset:{namespace:'http://b3mn.org/stencilset/bpmn2.0#'}}};
                        $rootScope.editor = new ORYX.Editor(d);
                        $rootScope.modelData = angular.fromJson(d);
                        $rootScope.editorFactory.resolve();*/
                    });
            }
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
	                ...
	                jQuery(window).resize(function () {
	                    ...
	                    var propSectionHeight = 1;//jQuery('#propertySection').height();
